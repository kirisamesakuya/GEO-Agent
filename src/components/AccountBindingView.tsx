...
export default function AccountBindingView({ brandName, embedded }: AccountBindingViewProps) {
  ...
  const handleBindConfirm = (acc: AccountBinding) => {
    ...
    if (!session) {
      toast('请先点击「打开平台登录页」并在新标签页完成登录', 'error');
      return;
    }
    setLoadingId(acc.id);
    fetch('/api/accounts/bind/confirm', {
      ...
      .then((data) => {
        ...
        toast(`${acc.platform} 已记录为本机登录确认`, 'success');
      })
      .finally(() => setLoadingId(null));
  };
  ...
  const handleUnbind = (id: string, platform: string) => {
    if (!confirm(`确定取消 ${platform} 的本机登录确认吗？取消后将无法自动发布到该平台。`)) return;
    setLoadingId(id);
    ...
    <div className={`${embedded ? 'h-full' : 'geo-page-content'} flex h-full min-h-0 overflow-hidden text-left`}>
      <div className="geo-brand-workspace flex-1 min-h-0">
        ...
        >
          ...
          {embedded && (
            <div className="px-6 py-3 shrink-0 flex justify-between items-center border-b" style={{ borderColor: 'var(--neutral-divider-02)' }}>
              <p className="text-xs" style={{ color: 'var(--neutral-text-03)' }}>
                浏览器登录须由您点击触发；完成登录后点击「我已在本机浏览器登录」。
              </p>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="text-xs" style={{ color: 'var(--neutral-text-03)' }}>
                已确认可发布 {authorizedCount}/{accounts.length}
              </span>
            </div>
                  ...
                  >
                    ...
                    <div className="geo-account-bind-body">
                      <div className="geo-account-bind-head">
                        ...
                        <span
                          className={`inline-flex shrink-0 text-[10px] font-bold ${bindingToLoginStatus(acc.status).tagClass}`}
                        >
                          {bindingToLoginStatus(acc.status).label}
                        </span>
                      </div>
                          ...
                          <>
                            ...
                            {showAuthorizing && (
                              <>
                                ...
                                >
                                  {publishAccountActionLabels(acc.status).confirmLogin}
                                </button>
                              </>
                            )}
                            {!isPending && !showAuthorizing && (
                              <>
                                ...
                                >
                                  ...
                                  />
                                  {publishAccountActionLabels(acc.status).recheck}
                                </button>
                                <button
                                  type="button"
                            ...
                            {acc.status === '校验失败' && (
                              ...
                              >
                                {publishAccountActionLabels(acc.status).retry}
                              </button>
                            )}
                          </>
                        )}
            ...
            <p
              ...
              style={{ color: 'var(--neutral-text-03)' }}
            >
              凭据由 Hermes 在本地校验，服务端不存储平台密码。点击「打开平台登录页」将在新标签页打开各平台官方登录页。
            </p>
          </div>
            ...
            <button
              ...
              onClick={() => {
                ...
                if (unauthorized.length === 0) {
                  toast('所有平台均已确认本机登录', 'info');
                  return;
                }
                toast(`尚有 ${unauthorized.length} 个平台未完成本机登录确认`, 'info');
              }}
              className="geo-btn-primary geo-btn-sm w-full flex items-center justify-center gap-2"
            >
              <ShieldCheck className="w-4 h-4" />
              <span>
                确认进度 {authorizedCount}/{accounts.length}
              </span>
            </button>
          </div>
        </div>
