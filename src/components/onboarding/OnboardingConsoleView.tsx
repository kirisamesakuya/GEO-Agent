...
export default function OnboardingConsoleView({ brandName, taskId, onNavigate }: Props) {
  ...
  const buildCheckMessage = (
    ...
    if (!health.ok && health.bound) {
      return {
        tone: 'warning' as const,
        message: `检测完成：设备已绑定，但 Hermes 当前离线。请打开客户端后再次检测。${health.detail ? `（${health.detail}）` : ''}`,
      };
    }
    ...
    <div className="geo-page-content overflow-y-auto h-full space-y-4">
      <div className="geo-card p-6 max-w-3xl mx-auto">
        ...
        <div className="mt-6 rounded-xl border border-[var(--color-border)] p-4 space-y-4">
          ...
          <p className="text-xs text-[var(--neutral-text-03)]">
            开发连调：在汇智爱马仕助手中开启 API Server（8642），GEO 通过本机接口直接调用任务，无需设备绑定码。
            商用 Pull 模式：在 Hermes 客户端输入绑定码完成设备绑定（与发布账号本机登录确认无关）。
          </p>

          {status?.apiGatewayOk && (
            <p className="text-xs text-green-800 bg-green-50 border border-green-200 rounded-lg px-3 py-2">
              开发直连已就绪，可直接执行 GEO 任务。
            </p>
          )}
          ...
          {!status?.apiGatewayOk && !status?.device && (
            <div className="rounded-lg border border-amber-200 bg-amber-50/50 p-3 space-y-2">
              <p className="text-xs font-medium">Step 3 · 商用设备绑定（无 8642 时）</p>
              <p className="text-xs text-[var(--neutral-text-03)]">
                若无法开启 API Server，可生成绑定码并在 Hermes 客户端「绑定 GEO 投放助手」中输入。
              </p>
              <button
                type="button"
                className="geo-btn-secondary geo-btn-xs"
                onClick={() => {
                  void createHermesBindToken().then((data) => setBindToken(data.token));
                }}
              >
                生成设备绑定码
              </button>
              {bindToken && (
                <p className="text-xs font-mono">
                  绑定码：<strong>{bindToken}</strong>（15 分钟有效，请在 Hermes 客户端输入）
                </p>
              )}
            </div>
          )}
