...
export default function PublishRecordDetailView({ brandName, recordId, onNavigate }: Props) {
  ...
  return (
    <div className="geo-page-content h-full overflow-y-auto px-6 py-4 space-y-4">
      <button
        type="button"
        className="geo-btn-secondary geo-btn-sm flex items-center gap-2"
        onClick={() => onNavigate?.('content_library', 'publish_records')}
      >
          ...
          <div className="flex flex-wrap gap-2">
            {record.contentItemId && (
              <button
                ...
                onClick={() => onNavigate?.('content_library', `content:${record.contentItemId}`)}
              >
                查看原文
              </button>
            )}
                ...
                <button
                  ...
                  onClick={() => {
                    if (record.contentItemId) {
                      onNavigate?.('content_library', `content:${record.contentItemId}`);
                    }
                  }}
                >
