...
export default function PublishScheduleView({ brandName, onBrandChange }: Props) {
  ...
  return (
    <div className="geo-page-content overflow-y-auto h-full space-y-4">
      <BrandScopeBar label="哪个品牌的发布计划" brandName={brandName} onBrandChange={onBrandChange} />
      <h2 className="text-lg font-bold">多项目发布排程</h2>
      <p className="text-xs text-[var(--neutral-text-03)]">
        选择多个 GEO 项目与文章，系统拆成单条 PublishJob，到点后由 Hermes 逐条执行（同账号串行）。
      </p>
        ...
        {selectedProjectIds.size > 0 && (
          <div>
            ...
            <p className="text-[11px] text-[var(--neutral-text-03)] mt-1">
              已选 {selectedItemIds.size} 篇 · 账号按平台自动匹配已授权绑定
            </p>
          </div>
        )}
