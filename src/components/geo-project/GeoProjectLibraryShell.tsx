...
export default function GeoProjectLibraryShell({
  ...
  const [section, setSection] = useState<ArticleResultSection>(
    initialSection ?? parseArticleResultSectionFromUrl()
  );
  ...
  return (
    <GeoListPageShell
      ...
      title="文章结果"
      description="生成文章 → 查看结果 → 发布 → 在发布记录中确认执行反馈。接单交付内容请在「任务交付」查看。"
      primaryAction={
        section === 'articles' ? (
          <button
            type="button"
      ...
      sectionTabs={ARTICLE_RESULT_SECTIONS}
      activeSection={section}
      onSectionChange={(id) => switchSection(id as ArticleResultSection)}
      statusTabs={
        section === 'articles'
          ? articleStatusTabs
      ...
      <div className="h-full min-h-0 geo-card overflow-hidden">
        {section === 'articles' ? (
          <ContentLibraryView
            ...
            onOpenArticleDetail={(id) => onNavigate?.('content_library', articleDetailHint(id))}
          />
        ) : brandName === '__all__' ? (
          ...
          <PublishRecordsPanel
            ...
            onNavigate={onNavigate}
            onViewRecord={(id) => onNavigate?.('content_library', publishRecordDetailHint(id))}
          />
        )}
      </div>
    </GeoListPageShell>
