...
export default function PlatformApp() {
  ...
  const load = () => {
    if (view === 'dashboard') {
      ...
      fetch('/api/platform/withdrawal-requests').then((r) => r.json()).then((d) => setWithdrawals(d.requests ?? []));
    ...
    <PlatformLayout
      ...
      drawer={(
        ...
        {selectedWebsiteRequest && view === 'website' && websiteTab === 'requests' && (
          <aside className="w-full lg:w-[420px] border-t lg:border-t-0 border-l overflow-y-auto p-4 space-y-4 shrink-0 max-h-[50vh] lg:max-h-none" style={{ borderColor: 'var(--neutral-divider-02)' }}>
    ...
    >
          ...
          {view === 'funds' && (
            <div className="space-y-4">
              ...
              <div className="platform-card p-4">
                <h3 className="font-semibold text-sm mb-3">接单方提现审核</h3>
                {withdrawals.filter((w) => w.status === 'pending' || w.status === 'approved').length === 0 ? (
                  <p className="text-xs" style={{ color: 'var(--neutral-text-03)' }}>无待处理提现</p>
                ) : (
                  withdrawals
                    .filter((w) => w.status === 'pending' || w.status === 'approved')
                    .map((w) => (
                      <div key={w.id} className="flex justify-between items-center py-2 border-b text-sm gap-3" style={{ borderColor: 'var(--neutral-divider-02)' }}>
                        <div className="min-w-0">
                          <p className="font-medium truncate">{w.provider?.name ?? w.providerId}</p>
                          <p className="text-xs" style={{ color: 'var(--neutral-text-03)' }}>
                            ¥{w.amount.toLocaleString('zh-CN')} · {w.channelLabel ?? w.channel} ·{' '}
                            {w.status === 'pending' ? '待审核' : '待打款'}
                          </p>
                        </div>
                        <div className="flex gap-2 shrink-0">
                          {w.status === 'pending' && (
                            <>
                              <button
                                ...
                                disabled={!can('funds.deposit')}
                                onClick={async () => {
                                  const res = await platformFetch(role, `/api/platform/withdrawal-requests/${w.id}/approve`, {
                                    method: 'POST',
                                    body: JSON.stringify({ role }),
                                  });
                                  const data = await res.json();
                              ...
                              <button
                                ...
                                disabled={!can('funds.deposit')}
                                onClick={async () => {
                                  const res = await platformFetch(role, `/api/platform/withdrawal-requests/${w.id}/reject`, {
                                    method: 'POST',
                                    body: JSON.stringify({ role, reason: '平台驳回' }),
                                  });
                                  const data = await res.json();
                                  if (data.error) { toast(data.error, 'error'); return; }
                                  toast('已驳回', 'success');
                          ...
                          {w.status === 'approved' && (
                            <button
                              ...
                              disabled={!can('funds.deposit')}
                              onClick={async () => {
                                const res = await platformFetch(role, `/api/platform/withdrawal-requests/${w.id}/mark-paid`, {
                                  method: 'POST',
                                  body: JSON.stringify({ role }),
                                });
                                const data = await res.json();
