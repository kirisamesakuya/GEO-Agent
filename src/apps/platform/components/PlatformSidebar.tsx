...
export default function PlatformSidebar({ view, onNavigate, open = false, onClose }: Props) {
  const { role, setRole, can } = usePlatformRole();

  return (
    <aside className={`platform-sidebar fixed left-0 top-0 z-20 flex h-screen shrink-0 flex-col border-r border-[var(--platform-border)] bg-white ${open ? 'platform-sidebar--open' : ''}`}>
      <div className="flex h-14 lg:h-[72px] items-center gap-3 border-b border-[var(--platform-border-subtle)] px-4 lg:px-5">
        <PlatformLogo />
        <div className="min-w-0 flex-1">
          <div className="truncate text-sm lg:text-base font-bold tracking-normal text-[var(--platform-text-title)]">GEO 投放助手</div>
        ...
        {onClose && (
          <button
            type="button"
            onClick={onClose}
            className="lg:hidden grid h-8 w-8 place-items-center rounded-md hover:bg-[var(--platform-surface-subtle)]"
            aria-label="关闭导航"
          >
            <X className="h-4 w-4" />
          </button>
