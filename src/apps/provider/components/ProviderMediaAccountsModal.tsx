    ...
    <div className="provider-modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="provider-media-modal-title">
      <div className="provider-modal max-w-lg w-full max-h-[90vh] flex flex-col">
        <div className="provider-modal-head flex items-start justify-between gap-3 shrink-0">
          <div>
            <h3 id="provider-media-modal-title" className="text-sm font-bold text-gray-900">
              绑定媒体账号
            </h3>
            <p className="text-xs text-gray-400 mt-1">填写各平台账号名称与主页链接，供平台审核与接单匹配</p>
          </div>
        ...
        <div className="provider-modal-body overflow-y-auto flex-1 min-h-0 space-y-4">
          {accounts.length === 0 ? (
            <p className="text-xs text-gray-400 text-center py-6">暂未添加账号，请在下方填写后点击「添加」</p>
          ) : (
              ...
              {accounts.map((a) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900">{a.accountName || '未命名'}</p>
                    <p className="text-xs text-gray-500 mt-0.5">{a.platform}</p>
                    {a.profileUrl && (
                      <a
                        href={a.profileUrl}
                        target="_blank"
          ...
          {!readOnly && (
            <div className="p-4 rounded-xl border border-dashed border-gray-200 space-y-3">
              <p className="text-xs font-semibold text-gray-700">{editingId ? '编辑账号' : '添加账号'}</p>
              <div>
                <label className="text-[10px] text-gray-500 block mb-1">平台</label>
                <select
                  value={platform}
                  onChange={(e) => setPlatform(e.target.value as (typeof PLATFORMS)[number])}
                  className="w-full px-3 py-2 border border-gray-100 rounded-lg text-sm bg-white"
                >
                  {PLATFORMS.map((p) => (
              ...
              <div>
                <label className="text-[10px] text-gray-500 block mb-1">账号名称 / 昵称 *</label>
                <input
                  value={accountName}
              ...
              <div>
                <label className="text-[10px] text-gray-500 block mb-1">主页链接（选填）</label>
                <input
                  value={profileUrl}
