    ...
    return (
      <div className="space-y-6">
        <button
          ...
          className="text-xs text-gray-400 hover:text-brand flex items-center gap-1"
        >
          ← 返回任务大厅
        </button>
        <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
          <div className="flex gap-6 flex-col lg:flex-row">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-500 mt-1">{selected.brandName} · ¥{selected.budget.toLocaleString()}</p>
              <p className="text-[10px] text-gray-400 mt-1">{formatTaskPublishedAt(selected.createdAt)}</p>
              {selected.matchLabel && (
                ...
                return (
                  <p className="text-xs text-gray-500 mt-2 flex items-center gap-1">
                    ...
                    {selected.slotTotal != null && selected.slotTotal > 1 && (
                      <span className="text-gray-400">（共 {selected.slotTotal} 名额）</span>
                    )}
                  </p>
                );
              })()}
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                <div className="p-3 bg-gray-50 rounded-xl">
                  <p className="text-xs text-gray-400 mb-1">交付物</p>
            ...
            <div className="shrink-0 flex flex-col items-center">
              ...
              <span className="text-[10px] text-gray-400 mt-2">匹配度</span>
            </div>
          </div>
          <p className="text-xs text-gray-500 mt-6 bg-gray-50 rounded-lg px-3 py-2">
          ...
          <textarea
            ...
            className="w-full mt-3 px-4 py-2 border border-gray-100 rounded-xl text-sm outline-none focus:border-gray-200"
          />
          <button
            type="button"
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs text-gray-400">公开任务可直接领取，领取后前往「我的订单」交付</p>
        </div>
        <div className="relative w-64">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-1.5 bg-white border border-gray-100 rounded-lg text-xs outline-none focus:border-brand/50"
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className={`text-xs px-3 py-1.5 rounded-lg font-medium ${
            platform === '' ? 'provider-nav-active' : 'provider-nav-item border border-gray-100'
          }`}
        >
          全部平台
        </button>
        {PROVIDER_TASK_HALL_FILTER_PLATFORMS.map((p) => (
          <button
            ...
            className={`text-xs px-3 py-1.5 rounded-lg font-medium ${
              platform === p ? 'provider-nav-active' : 'provider-nav-item border border-gray-100'
            }`}
          >
            {p}
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {filtered.length === 0 ? (
          <p className="text-sm text-gray-400 col-span-2 py-8">暂无可领取任务</p>
        ) : (
          filtered.map((t) => {
            ...
            <button
              ...
              className={`bg-white border rounded-2xl p-5 text-left hover:shadow-md transition-all group ${
                slots.isFull
                  ? 'border-gray-100 opacity-75 hover:border-gray-200'
            ...
            >
              <div className="flex gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-bold text-gray-900 truncate group-hover:text-brand">{t.title}</h3>
                  <p className="text-xs text-gray-400 mt-1">{t.brandName}</p>
                  <p className="text-[10px] text-gray-400 mt-0.5">{formatTaskPublishedAt(t.createdAt)}</p>
                  <div className="flex flex-wrap items-center gap-2 mt-2">
                    ...
                    <span
                      className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${
                        slots.isFull ? 'bg-gray-100 text-gray-500' : 'bg-emerald-50 text-emerald-700'
                      }`}
                    >
                      {slots.availableText}
                    </span>
                ...
                <ChevronRight className="w-5 h-5 text-gray-300 group-hover:text-brand shrink-0" />
              </div>
            </button>
            );
          })
