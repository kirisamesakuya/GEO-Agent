...
export default function ProviderAccountView({ providerId }: Props) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-gray-900">账号资源</h1>
      ...
      <div className={`grid gap-4 ${PROVIDER_CASE_SUBMISSION_ENABLED ? 'grid-cols-3' : 'grid-cols-2'}`}>
        <div className="bg-white rounded-2xl border border-gray-100 p-4 text-center shadow-sm">
          ...
          <p className="text-xs text-gray-400 mt-1">已绑定平台</p>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 p-4 text-center shadow-sm">
          <p className="text-2xl font-bold text-brand">{canAcceptCount}</p>
          <p className="text-xs text-gray-400 mt-1">可接单资源</p>
        </div>
        {PROVIDER_CASE_SUBMISSION_ENABLED && (
          <div className="bg-white rounded-2xl border border-gray-100 p-4 text-center shadow-sm">
            <p className="text-2xl font-bold text-gray-900">{assets.length}</p>
            <p className="text-xs text-gray-400 mt-1">案例素材</p>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {platforms.map((p) => (
          <div key={p} className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm flex justify-between items-start">
            <div>
              <div className="w-10 h-10 rounded-xl bg-brand flex items-center justify-center text-white text-xs font-bold mb-3">
                {p.slice(0, 2)}
              </div>
              <h3 className="font-bold text-gray-900">{p} 服务资源</h3>
              <p className="text-xs text-gray-400 mt-1">已启用接单匹配</p>
              <div className="flex flex-wrap gap-1 mt-2">
                {industryTags.slice(0, 3).map((t) => (
                  <span key={t} className="text-[9px] bg-gray-100 text-gray-500 px-2 py-0.5 rounded">
                    {t}
                  </span>
                ))}
              </div>
        ...
        {platforms.length === 0 && (
          <p className="text-sm text-gray-400 col-span-2">请在下方勾选至少一个平台</p>
        )}
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm space-y-5">
        ...
        <div>
          <label className="text-xs text-gray-500 block mb-2">擅长平台</label>
            ...
            {PLATFORMS.map((p) => (
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-lg font-medium ${
                  platforms.includes(p) ? 'provider-nav-active' : 'provider-nav-item border border-gray-100'
                }`}
              >
                {p}
              </button>
        ...
        <div>
          <label className="text-xs text-gray-500 block mb-2">行业标签</label>
          <div className="flex flex-wrap gap-2">
            {INDUSTRIES.map((t) => (
              <button
                key={t}
                ...
                className={`text-xs px-3 py-1.5 rounded-lg font-medium ${
                  industryTags.includes(t) ? 'provider-nav-active' : 'provider-nav-item border border-gray-100'
                }`}
              >
                {t}
              </button>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs text-gray-500 block mb-1">报价下限 ¥</label>
            <input
              type="number"
              value={budgetMin}
          ...
          <div>
            <label className="text-xs text-gray-500 block mb-1">报价上限 ¥</label>
            <input
              type="number"
              value={budgetMax}
        ...
        {PROVIDER_CASE_SUBMISSION_ENABLED && (
          <div>
            <label className="text-xs text-gray-500 block mb-1">案例链接</label>
            <textarea
              value={caseLinks}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm space-y-4">
        ...
        {showAddAsset && (
          <div className="flex gap-2 flex-wrap p-3 bg-gray-50 rounded-xl">
            <input
              value={assetTitle}
              onChange={(e) => setAssetTitle(e.target.value)}
              placeholder="标题"
              className="flex-1 min-w-[100px] px-3 py-2 border border-gray-100 rounded-lg text-xs"
            />
            <input
              value={assetUrl}
              onChange={(e) => setAssetUrl(e.target.value)}
              placeholder="链接 URL"
              className="flex-[2] min-w-[140px] px-3 py-2 border border-gray-100 rounded-lg text-xs"
            />
            <button
              type="button"
        ...
        {assets.map((a) => (
          <div key={a.id} className="flex justify-between items-center text-sm py-2 border-b border-gray-50 last:border-0">
            <a href={a.url} target="_blank" rel="noreferrer" className="text-brand truncate flex-1">
              {a.title}
            </a>
            <button
              type="button"
              className="text-xs text-gray-400 hover:text-brand ml-2"
              onClick={async () => {
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm space-y-3">
        <h2 className="text-sm font-bold text-gray-900">报价规则</h2>
        {pricingRules.map((r) => (
          <div key={r.id} className="flex justify-between text-sm p-3 bg-gray-50 rounded-xl">
            <span>
        ...
        <div className="flex flex-wrap gap-2">
          <input
            ...
            placeholder="任务类型"
            className="px-3 py-2 border border-gray-100 rounded-lg text-xs"
          />
          <input
            value={ruleMin}
            onChange={(e) => setRuleMin(e.target.value)}
            placeholder="最低"
            className="w-20 px-3 py-2 border border-gray-100 rounded-lg text-xs"
          />
          <input
            value={ruleMax}
