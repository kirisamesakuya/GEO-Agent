...
export default function ProviderProfileView({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm flex flex-col md:flex-row gap-6">
        <img src={DEFAULT_AVATAR} alt="" className="w-20 h-20 rounded-2xl border border-gray-100 object-cover" />
      ...
      {!approved && (
        <section className="space-y-4">
          <div>
            <h2 className="text-sm font-bold text-gray-900">入驻审核进度</h2>
            <p className="text-xs text-gray-400 mt-0.5">填写资料、提交审核并等待平台确认</p>
          </div>
          <ProviderOnboarding
            providerId={providerId}
            onProviderReady={onProviderReady}
      ...
      {approved && (
        <div className="bg-brand-light/30 border border-brand-light rounded-2xl p-4 text-sm text-gray-700">
          您已通过平台入驻审核，可在任务大厅直接领取合作任务。
        </div>
      )}

      <div className="bg-white rounded-2xl border border-dashed border-gray-200 p-5 shadow-sm">
        <h3 className="text-sm font-bold text-gray-900 mb-1">演示与体验</h3>
