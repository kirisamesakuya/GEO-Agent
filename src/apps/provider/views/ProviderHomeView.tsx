...
export default function ProviderHomeView({
  ...
  return (
    <div className="space-y-6">
      <section className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-xl font-bold text-gray-900 mb-1">欢迎回来，{providerName}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <button
          ...
          className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm text-left h-[120px] hover:shadow-md transition-all group"
        >
          <span className="text-xs text-gray-500 block mb-1 font-medium">进行中的合作</span>
          <span className="text-2xl font-bold text-gray-900 font-mono">
            {activeOrders.length} <span className="text-xs font-normal text-gray-400">个订单</span>
          </span>
          <div className="flex justify-between text-[11px] text-gray-400 mt-2">
        ...
        <button
          ...
          className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm text-left h-[120px] hover:shadow-md transition-all group"
        >
          <span className="text-xs text-gray-500 block mb-1 font-medium">绑定的服务资源</span>
          <span className="text-2xl font-bold text-gray-900 font-mono">
            {platformCount} <span className="text-xs font-normal text-gray-400">个平台</span>
          </span>
          <div className="flex justify-between text-[11px] text-gray-400 mt-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <div className="flex justify-between items-center">
            ...
            <button type="button" onClick={() => onNavigate('tasks')} className="text-xs text-gray-400 hover:text-brand flex items-center gap-0.5">
              全部任务 <ArrowRight className="w-3 h-3" />
            </button>
          </div>
          <div className="space-y-3.5">
            {recommendedTasks.length === 0 ? (
              <p className="text-xs text-gray-400 py-4">暂无推荐任务</p>
            ) : (
              recommendedTasks.map((task) => (
                <button
                  ...
                  className="w-full bg-white border border-gray-100 p-4 rounded-xl flex items-center gap-4 hover:border-brand/40 hover:shadow-sm transition-all text-left group"
                >
                  <div
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-sm font-bold text-gray-900 truncate group-hover:text-brand">{task.title}</h3>
                    <p className="text-[11px] text-gray-400 mt-1">品牌：{task.brandName}</p>
                    <p className="text-[10px] text-gray-400">{formatTaskPublishedAt(task.createdAt)}</p>
                    {task.industry && (
                      <span className="inline-block mt-2 bg-gray-100 text-gray-500 px-2 py-0.5 rounded text-[9px]">{task.industry}</span>
                    )}
                  </div>
                  <div className="text-right shrink-0">
                    <span className="text-sm font-black text-brand">¥{task.budget.toLocaleString()}</span>
                    {task.matchScore != null && (
                      <p className="text-[10px] text-gray-400 mt-1">匹配 {task.matchScore}%</p>
                    )}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm space-y-4">
          <div className="flex justify-between items-center">
            ...
            <button type="button" onClick={() => onNavigate('orders')} className="text-xs text-gray-400 hover:text-brand">
              查看全部
            </button>
          </div>
          {dashboard.todos.length === 0 && activeOrders.length === 0 ? (
            ...
            <div className="space-y-3">
              {dashboard.todos.map((t) => (
                <div key={t.type} className="text-xs p-2 rounded-lg bg-brand-light/30 text-gray-700">
                  {t.label}
                </div>
              ))}
              {activeOrders.map((order) => (
                <button
                  ...
                  className="w-full flex gap-3 hover:bg-gray-50/50 p-2 rounded-xl text-left transition-all"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-xs font-bold text-gray-800 truncate">{order.title}</div>
