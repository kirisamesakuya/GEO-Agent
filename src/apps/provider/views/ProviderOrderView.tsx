  ...
  return (
    <div className="space-y-4 -m-2">
      ...
      <div className="provider-split-layout flex gap-4 min-h-[480px]">
            ...
            {filtered.map((o) => (
              ...
              >
                <div className="flex justify-between gap-2 mb-1">
                  <span className="text-sm font-medium truncate text-provider-title">{o.title}</span>
                  <span className={`text-[9px] px-1.5 py-0.5 rounded shrink-0 ${ORDER_STATUS_CHIP[o.status] ?? 'bg-provider-hover'}`}>
                    {ORDER_STATUS_LABEL[o.status] ?? o.status}
                  </span>
                </div>
                <span className="text-xs text-provider-muted">
        ...
        <div className="flex-1 provider-card rounded-2xl p-6 overflow-y-auto min-h-0">
          {!selected ? (
            <p className="text-sm text-provider-muted py-12 text-center">选择左侧订单查看详情</p>
          ) : (
            <div className="space-y-5">
              <div>
                <h2 className="text-lg font-bold text-provider-title">{selected.title}</h2>
              ...
              {selected.deliveries?.map((d, i) => {
                ...
                return (
                  <div key={i} className="p-4 border border-provider rounded-xl text-sm space-y-2">
                    <p className="text-xs text-provider-muted flex items-center gap-1 flex-wrap">
                      ...
                      <span className="text-provider-muted">· {new Date(d.createdAt).toLocaleString('zh-CN')}</span>
                    </p>
                    {d.reviewNote && (
                      <p className="text-xs text-amber-800 bg-amber-50 rounded-lg px-2 py-1">修改意见：{d.reviewNote}</p>
                    )}
                    <pre className="whitespace-pre-wrap text-provider-body">{d.content}</pre>
                    {d.link && (
              ...
              <div className="p-4 bg-blue-50/50 border border-blue-100 rounded-xl text-xs text-provider-secondary">
              ...
              {selected.status === 'revision' && !isArticleContentOrder(selected) && (
                <div className="space-y-2">
                  <p className="text-xs font-medium text-provider-body">提交返修响应</p>
                  <textarea
                    value={revisionResponse}
                    onChange={(e) => setRevisionResponse(e.target.value)}
                    rows={4}
                    className="w-full px-4 py-2 border border-provider rounded-xl text-sm"
              ...
              {isArticleContentOrder(selected) &&
                ...
                <div className="space-y-3 border-t border-provider pt-4">
                  <p className="text-sm font-bold text-provider-title flex items-center gap-2">
                  ...
                  {deliveryAttachments.length > 0 && (
                    <ul className="text-xs space-y-1">
                      {deliveryAttachments.map((a, i) => (
                        <li key={i} className="flex justify-between text-provider-secondary">
                          <span>{a.name}</span>
              ...
              {isArticleContentOrder(selected) && selected.status === 'revision' && (
                <div className="space-y-3 border-t border-provider pt-4">
                  <p className="text-sm font-bold text-provider-title">最终验收返修 — 重新提交发布交付</p>
                  <textarea
                    value={deliveryContent}
                    onChange={(e) => setDeliveryContent(e.target.value)}
                    rows={3}
