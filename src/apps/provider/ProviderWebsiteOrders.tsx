  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-xs text-provider-muted mt-1">认领网页设计/技术交付订单，提交预览链接</p>
      </div>

      <div className="provider-split-layout flex gap-4 min-h-[420px]">
        <div className="provider-split-list w-full lg:w-72 shrink-0 provider-card rounded-2xl overflow-hidden">
          <div className="overflow-y-auto max-h-[520px]">
            {orders.length === 0 ? (
              <p className="p-4 text-xs text-provider-muted">暂无网站订单</p>
            ) : (
              orders.map((o) => (
                <button
                  key={o.id}
                  ...
                  className={`w-full text-left px-4 py-3 border-b border-provider transition-colors ${
                    selected?.id === o.id ? 'bg-brand-light/50' : 'hover:bg-provider-hover'
                  }`}
                >
                  <p className="text-sm font-medium truncate text-provider-title">{o.request?.pageType ?? '网页'}</p>
                  <p className="text-xs mt-1 text-provider-muted">
        ...
        <div className="flex-1 provider-card rounded-2xl p-6 overflow-y-auto">
          {!selected ? (
            <p className="text-sm text-provider-muted py-12 text-center">选择网站订单</p>
          ) : (
            <div className="space-y-4">
              <div>
                <h3 className="font-bold text-provider-title">{selected.request?.pageType ?? '网站订单'}</h3>
                <p className="text-xs text-provider-muted mt-1">
                  {selected.brandName} · {selected.status}
                </p>
                <p className="text-sm mt-3 text-provider-body">{selected.request?.goal}</p>
              </div>
              {selected.request?.previewHtml && (
                <div className="rounded-xl border border-provider overflow-hidden">
                  <div className="px-3 py-2 text-xs font-medium border-b border-provider bg-provider-subtle">AI 预览参考</div>
