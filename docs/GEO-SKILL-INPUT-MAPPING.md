...
| `geo_technical` | `geo-technical` | API / 后续 GeoAudit 模块拆分 | P0 已注册 taskType |
| `geo_crawlers` | `geo-crawlers` | API / 后续 GeoAssets | P0 已注册 taskType |
| `geo_content` | `geo-content` | API / 后续内容库 | P0 已注册 taskType |
| `geo_platform_optimizer` | `geo-platform-optimizer` | API / 后续排名监控 | P0 已注册 taskType |
...
- [x] 新增 taskType：`geo_technical`、`geo_crawlers`、`geo_content`、`geo_platform_optimizer`
- [x] 输出契约 `geoWebOutput.v1`（`server/lib/geo-web-output-contract.ts`）
- [x] `GET /api/hermes/geo-capabilities` 与 readiness 四态

### P1（后续）

- [ ] GeoAuditView 模块单跑、GeoAssets 扩展 crawlers/robots — **已完成 P1**
- [ ] 报告历史 geo_compare JSON 对比 — **已完成 P1**
- [ ] 内容库 geo_content / keyword_mining 确认流 — **已完成 P1**
