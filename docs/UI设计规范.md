...
| 断点 | 内容区内边距 | 侧栏行为 |
|------|-------------|----------|
| ≥ 1024px | `--sp-xl` (32px) | 固定 240px 侧栏 |
| 640–1023px | `--sp-lg` (24px) | 抽屉侧栏 + 遮罩 |
| < 640px | `--sp-base` (16px) | 抽屉侧栏；Header 精简 |

其他规则：

- 页面壳层不设固定 `min-width`，允许小屏纵向滚动
- 左右分栏（内容库预览、列表+详情）在 `< 1024px` 改为上下堆叠
- 卡片网格：`auto-fit, minmax(min(360px, 100%), 1fr)`
- 工作台 KPI 网格：4 列 → 2 列（720px）→ 1 列
...
| 类型 | 样式 | 用途 |
|------|------|------|
| 主操作 CTA | 青绿 `--color-accent` + 白字 | 页面主操作（提交 Hermes、确认发布、确认入库） |
| 品牌按钮 | 深蓝 `--color-primary` + 白字 | 品牌强调、信息类主操作 |
| 次按钮 | 白底 + 灰边框 + 深灰字 | 次要操作（取消、返回） |
| 文字按钮 | 无背景无边框 + 灰字 | 折叠、关闭、辅助操作 |
| 危险按钮 | 红底 + 白字 | 删除、退出等破坏性操作 |

> **双轨色彩**：链接、表头、Logo 底使用 `--color-primary`（深蓝）；页面主 CTA 使用 `--color-accent`（青绿），与 PRD 高保真一致。详见 `docs/design-framework.md`。
