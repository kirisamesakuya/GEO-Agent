...
import { skillNameForTaskType } from '../server/lib/agent-skill.js';
...
for (const [type, skill] of [
  ['geo_technical', 'geo-technical'],
  ['geo_crawlers', 'geo-crawlers'],
  ['geo_content', 'geo-content'],
  ['geo_platform_optimizer', 'geo-platform-optimizer'],
] as const) {
  const mapped = skillNameForTaskType(type);
  if (mapped !== skill) {
    throw new Error(`skillNameForTaskType(${type}) = ${mapped}, expected ${skill}`);
  }
  const task = await createAgentTask({
    ...
    executor: 'direct_model',
    input: {
      brandUrl: `https://www.verify-${type}.cn`,
      pageUrls: ['https://www.verify-example.cn/about'],
      queries: ['测试问题'],
      contentItems: [{ title: '页面', body: '正文' }],
    },
  });
  check(`createAgentTask ${type}`, task.input);
}
