...
export function registerContentRoutes(app: Express) {
  app.get('/api/content-batches', async (req, res) => {
    const { brandName, platform, projectId, limit } = req.query;
  ...
  app.post('/api/content-batches/items/bulk-delete', async (req, res) => {
    const { itemIds, brandName } = req.body ?? {};
  ...
  app.post('/api/content-batches/:id/confirm-publish', async (req, res) => {
    ...
    try {
      const task = await confirmBatchPublish(req.params.id, brandName);
  ...
  app.post('/api/content-batches/:id/hermes-publish', async (req, res) => {
    const { brandName, contentItemIds, accountBindingId } = req.body ?? {};
    if (!brandName) return res.status(400).json({ error: '缺少 brandName' });
  ...
  app.post('/api/content-batches/:id/publish-draft', async (req, res) => {
    const { brandName, contentItemIds, accountBindingId, frequency, runCount, autoComment } = req.body ?? {};
    if (!brandName) return res.status(400).json({ error: '缺少 brandName' });
  ...
  app.post('/api/content-batches', async (req, res) => {
    const { brandName, platform, articles, taskId, status } = req.body ?? {};
    if (!brandName || !platform || !articles?.length) return res.status(400).json({ error: '缺少必填字段' });
