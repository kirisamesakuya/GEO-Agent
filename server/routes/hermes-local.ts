...
/**
 * DEMO_ONLY:
 * Hermes 本机客户端协议接口，当前以 deviceIdHash / bindToken 识别设备。
 *
 * PRODUCTION_TODO:
 * 生产环境应增加设备绑定校验、请求签名、IP 限制与任务级权限校验。
 */
export function registerHermesLocalRoutes(app: Express) {
  ...
  app.post('/api/hermes-local/heartbeat', async (req, res) => {
    try {
      ...
      const result = await recordHermesHeartbeat({
        ...
        maxConcurrentTasks:
          maxConcurrentTasks != null ? Number(maxConcurrentTasks) : undefined,
        runningTaskIds: Array.isArray(runningTaskIds)
          ? runningTaskIds.map(String)
          : undefined,
        availableSlots: availableSlots != null ? Number(availableSlots) : undefined,
      });
      res.json(result);
    } catch (err) {
  ...
  app.get('/api/hermes-local/tasks/poll', async (req, res) => {
    try {
      ...
      const result = await pullHermesTasksPoll(deviceIdHash, limit);
      res.json(result);
    } catch (err) {
      res.status(400).json({ error: err instanceof Error ? err.message : '批量拉取任务失败' });
    }
  });
