...
export function registerWebsiteRoutes(app: Express) {
  ...
  app.post('/api/website-requests', async (req, res) => {
    const { brandName, brand, pageType, goal, referenceUrl, modules, attachments } = req.body ?? {};
    const name = brandName ?? brand;
    if (!name || !pageType || !goal) {
      return res.status(400).json({ error: '缺少品牌、页面类型或目标' });
    }
  ...
  app.post('/api/website-requests/preview', async (req, res) => {
    ...
    const gate = await validateAgentTaskSubmission(name, 'website_preview', 15);
    if (!gate.ok) return res.status(400).json({ error: gate.error });

    const task = await createAgentTask({
      type: 'website_preview',
      title: `${name} · ${pageType ?? '网页'}预览`,
      brandName: name,
      input: {
        brand: name,
        pageType,
