...
export async function previewAgentTaskResult(taskId: string) {
  ...
  if (task.type === 'keyword_mining') {
    const suggestions =
      (task.output?.suggestions as Array<{ term: string; group: string }> | undefined) ?? [];
    return {
      task,
      status,
      preview: {
...
export async function confirmAgentTaskResult(
  ...
  if (task.type === 'brand_extract') {
    ...
    if (input.groupOverrides) {
      items = items.map((s) => ({
        ...s,
        group: input.groupOverrides![s.term] ?? s.group,
      }));
    }
...
export async function markTaskPendingResultConfirmation(
  task: AgentTask,
  confirmationType: 'brand_profile' | 'keyword_suggestions' | 'knowledge_entries'
) {
