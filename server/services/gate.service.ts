...
import { getGeoCapabilities, isTaskTypeUnavailable } from '../lib/geo-capabilities.js';

const GEO_BRAND_PROFILE_TASK_TYPES = new Set([
  'geo_analysis',
  'geo_quick_start',
  'geo_audit',
  'geo_technical',
...
export async function validateAgentTaskSubmission(
  ...
  if (!complete && GEO_BRAND_PROFILE_TASK_TYPES.has(taskType)) {
  ...
  const capabilityGate = isTaskTypeUnavailable(taskType, capabilities);
  if (capabilityGate.unavailable) {
    return {
      ok: false,
      error: capabilityGate.reason ?? '当前 GEO skill 不可用，请检查 Hermes 安装与 API Server 状态',
    };
  }
...
export async function validateAutoPublish(
  ...
  if (!acc || acc.status !== '已授权') {
    return {
      ok: false,
      error: `${platform} 未完成本机登录确认。请在本机浏览器登录该平台后台，并在「发布账号」页手动确认`,
    };
  }
  return { ok: true };
}
