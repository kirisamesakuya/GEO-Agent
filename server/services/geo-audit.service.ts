...
export async function createGeoReportFromTaskOutput(
  ...
  const taskReportTypeMap: Partial<Record<string, string>> = {
    geo_quick_start: 'quick_start',
    geo_audit: 'audit',
    geo_technical: 'technical',
    geo_crawlers: 'crawlers',
    geo_content: 'content',
    geo_platform_optimizer: 'platform_optimizer',
    geo_compare: 'compare',
    geo_schema: 'assets',
    geo_llmstxt: 'assets',
    geo_citability: 'assets',
  };
  const reportType = String(
    audit.reportType ?? taskReportTypeMap[task.type] ?? 'analysis'
  );
