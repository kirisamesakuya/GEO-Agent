...
import { startAccountVerifyTask } from './brand.service.js';
import {
  effectiveAuthMode,
...
export async function confirmAccountBind(
  ...
  if (bindSessionId !== binding.bindSessionId) {
    throw new Error('授权会话已过期，请重新点击「去授权」');
  }
  if (binding.status !== '授权中') {
    throw new Error('当前状态无法确认登录，请重新发起授权');
  }
  ...
  await appendBindingLog(accountId, 'bind_confirm', bindSessionId);
  await appendAuditLog({
    action: 'account_bind_confirm',
    entity: 'AccountBinding',
    entityId: accountId,
    detail: binding.platform,
  });

  return { task, accounts: await listAccounts(name) };
}
