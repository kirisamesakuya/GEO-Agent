...
export async function getOnboardingStatus(brandName?: string) {
  ...
  const geoCapabilities: GeoCapabilitiesResponse = await getGeoCapabilities(health);
  const readinessUiState: ReadinessUiState = resolveReadinessUiState({
    hermesReady,
    setupReason,
    capabilities: geoCapabilities,
  });

  const degradedSkills = geoCapabilities.skills
    .filter((s) => s.status === 'degraded')
    .map((s) => s.name);
  const unavailableSkills = geoCapabilities.skills
    .filter((s) => s.status === 'unavailable')
    .map((s) => s.name);

  const apiGatewayOk = health.apiGatewayOk ?? false;
  const connectionMode: HermesConnectionMode = resolveHermesConnectionMode({
    apiGatewayOk,
    deviceBound: Boolean(device),
  ...
  return {
    ...
    readinessUiState,
    geoCapabilities: {
      hermesVersion: geoCapabilities.hermesVersion,
      geoSkillsVersion: geoCapabilities.geoSkillsVersion,
      source: geoCapabilities.source,
      registeredTaskTypes: geoCapabilities.skills.flatMap((s) => s.taskTypes),
      skillCount: geoCapabilities.skills.length,
      degradedSkills,
      unavailableSkills,
      workspace: geoCapabilities.workspace,
    },
  };
}
