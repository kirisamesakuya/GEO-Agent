...
/** 开发演示：允许无 Hermes 时自动走 GEO fixture Mock */
export function isGeoSkillMockDemoMode(): boolean {
  return process.env.GEO_SKILL_MOCK_DEMO === 'true';
}
...
export function allowsDirectModelGeoFixtureMock(task: {
  ...
  if (!isGeoSkillMockDemoMode() && !input.userConfirmedExecution) return false;
